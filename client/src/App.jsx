...
const LogoutButton = () => {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('token'); // Bổ sung xóa token cho an toàn
    navigate('/login');
  };
  return (
    <button onClick={handleLogout} style={{ background: 'transparent', color: '#EF4444', border: '1px solid #EF4444', padding: '5px 15px', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer', marginLeft: '10px' }}>
      ĐĂNG XUẤT
    </button>
  );
};
    ...
    <div className="layout-container">
      <nav className="navbar">
        ...
        <div className="navbar-menu" style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          {/* NÚT BÍ MẬT CHỈ HIỆN KHI LÀ ADMIN */}
          {isAdmin && (
            <Link to="/admin" style={{color: '#EF4444', textDecoration: 'none', fontWeight: 'bold', border: '1px dashed #EF4444', padding: '3px 8px', borderRadius: '4px'}}>
              QUẢN TRỊ ADMIN
            </Link>
          )}

          <button className="theme-toggle" onClick={toggleTheme} style={{ cursor: 'pointer', background: '#334155', color: 'white', border: 'none', padding: '5px 10px', borderRadius: '4px', fontWeight: 'bold' }}>
    ...
    <div className="layout-container" style={{ borderTop: '5px solid #EF4444' }}>
      <nav className="navbar" style={{ background: '#0F172A' }}>
        ...
        <div className="navbar-menu" style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          {/* NÚT QUAY LẠI TRANG GAME CHO ADMIN */}
          <Link to="/" style={{color: '#10B981', textDecoration: 'none', fontWeight: 'bold', border: '1px solid #10B981', padding: '3px 8px', borderRadius: '4px'}}>
            CHƠI GAME
          </Link>
          
          <Link to="/admin" style={{color: 'white', textDecoration: 'none', fontWeight: 'bold'}}>DASHBOARD</Link>
          <LogoutButton />
        </div>
      </nav>
